...
significance = 0.0001 # significance level for CSR testing (used by statsmodels but not affecting the output, must be <= 1/permutations)

# spatial weights matrix creation
w = psw.Queen.from_dataframe(df) # edge-corner contiguity

'''# alternative spatial weights - use only one
w = psw.Rook.from_dataframe(df) # corner contiguity
w = psw.Kernel.from_dataframe(df, bandwidth = 1, function = 'gaussian') # Kernel weights
'''
